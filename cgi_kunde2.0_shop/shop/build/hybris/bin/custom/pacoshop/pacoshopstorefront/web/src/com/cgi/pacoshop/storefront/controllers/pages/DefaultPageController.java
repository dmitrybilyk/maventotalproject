...
public class DefaultPageController extends AbstractPageController
{
	...
	public String get(final Model model, final HttpServletRequest request, final HttpServletResponse response) throws CMSItemNotFoundException
	{
		...
		ContentPageModel pageForRequest = null;
		try
		{
			pageForRequest = getContentPageForRequest(request);
		}
		catch (CMSItemNotFoundException cmsException)
		{
			request.setAttribute(EXCEPTION, new PageNotFoundException(Arrays.toString(cmsException.getStackTrace())));
		}
