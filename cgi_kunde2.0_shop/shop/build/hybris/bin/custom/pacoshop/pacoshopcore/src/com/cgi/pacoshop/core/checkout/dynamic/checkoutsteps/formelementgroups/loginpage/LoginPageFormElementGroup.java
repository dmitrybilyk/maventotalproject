...
public class LoginPageFormElementGroup extends AbstractFormElementGroup
{
    ...
    private boolean isAsAnonymousVisitorSubmitted(final CartModel cartModel)
    {
        ...
        {
            if (groupModel.getUid().equals(getConfigurationProperty(
                    ShopConfigurationService.CONFIG_KEY_VISITOR_CUSTOMER_GROUP_UID))
    ...
    {
        getSession().setAttribute(FormElementGroupConstants.ANONYMOUS_SUBMITTED, true);
    }

    private boolean isAsAnonymousSubmitted()
    {
        Object buttonClicked = getSession().getAttribute(FormElementGroupConstants.ANONYMOUS_SUBMITTED);
