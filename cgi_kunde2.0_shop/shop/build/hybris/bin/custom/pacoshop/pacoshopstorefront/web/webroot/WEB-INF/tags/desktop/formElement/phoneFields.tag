...
<%@ attribute name="pathExtension"			required="true"		rtexprvalue="true" %>
<%@ attribute name="pathNumber"			required="true"		rtexprvalue="true" %>
<%@ attribute name="idPrefix"				required="true"		rtexprvalue="true" %>
<%@ attribute name="idExtension"				required="true"		rtexprvalue="true" %>
...
<%@ attribute name="placeholderExtension"	required="false"	rtexprvalue="true" %>
<%@ attribute name="placeholderExtensionKey"	required="false"	rtexprvalue="true" %>
...
<c:if test="${not empty placeholderExtensionKey}">
    <spring:theme code="${placeholderExtensionKey}" var="placeholderExtension" />
</c:if>
...
<spring:bind path="${pathExtension}">
    <c:set var="msgClassExtension" value="" />
    <c:if test="${status.error}">
        <c:set var="msgClassExtension" value=" has-msg has-msg-${status.error ? 'error' : ''}" />
    </c:if>
</spring:bind>
...
<c:if test="${msgClassPrefix ne '' || msgClassExtension ne '' || msgClassNumber ne '' }">
...
<li class="has-dateFields has-field-${pathPrefix} ${msgClass} ${requiredClass} ${cssClass}">
        ...
        <span class="field multiple multiple-3">
            <form:input maxlength="5" type="tel" pattern="[+]?[0-9]*" id="${idPrefix}" path="${pathPrefix}" placeholder="${placeholderPrefix}" cssClass="${cssClass}"/>
            <form:input maxlength="5" type="tel" pattern="[( -)0-9]*" id="${idExtension}" path="${pathExtension}" placeholder="${placeholderExtension}" cssClass="${cssClass}"/>
            <form:input maxlength="8" type="tel" pattern="[( -)0-9]*" id="${idNumber}" path="${pathNumber}" placeholder="${placeholderNumber}" cssClass="${cssClass}"/>
        </span>
        <span class="msg ${msgType}">
            <span><form:errors path="${pathPrefix}"/><form:errors path="${pathExtension}"/><form:errors path="${pathNumber}"/></span>
        </span>
</li>
